trigger SPProductObjectTrigger on SPProduct__c (after delete, after insert, after undelete, after update, 
    before delete, before insert, before update) {
    if(Label.ExecuteSPTrigger=='true' && SIConstants.allowSPTrigger == true){
    
    boolean isExecuted = false;
    SITriggerOperations.SITriggerImpl handler = new SITriggerDelegate().getHandlerHandleObject(SIConstants.PRO_TRIG_HANDLER);
